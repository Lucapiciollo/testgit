...
export default function addElement(sezione, sezNumero) {
  ...
  btnTitle.setAttribute('data-bs-target', '#panelsStayOpen-element' + (numElement + 1));
  let btnTitletext = document.createTextNode('Elemento 1 di sezione ' + (sezNumero));
  let divBody = document.createElement('div');
  divBody.setAttribute('id', 'panelsStayOpen-element' + (numElement + 1));
  ...
  let attaccaA = document.getElementById('footerSez' + sezNumero);
  attaccaA.insertAdjacentElement('beforebegin', divHeadler);
  selected();
